...
// ensure `annual` column exists (1 = repeats every year)
try {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] === 'add' && !empty($_POST['date'])) {
        ...
        $stmt = $pdo->prepare('REPLACE INTO holidays (year,date,label,type,annual) VALUES (?,?,?,?,?)');
        $stmt->execute([$y, $d, $label, $type, $annual]);
...
<div class="container">
  <div class="card">
    ...
    <form method="post" class="form-wrapper">
      ...
      <div class="form-grid">
        <div class="form-group"><label class="form-label">Fecha</label><input class="form-control" type="date" name="date" required></div>
