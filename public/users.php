...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['reset_user_id'])){
  ...
  // Sin contraseña indicada → generar una temporal ALEATORIA (ya no un literal predecible).
  if ($new_password === '') {
    $new_password = bin2hex(random_bytes(5)); // 10 caracteres hex
    $_SESSION['flash_reset_pw'] = $new_password; // se muestra una vez al admin
  }
...
<div class="container">
  <?php if (!empty($_SESSION['flash_reset_pw'])): ?>
  <div class="card" style="border-left:4px solid var(--success-color);">
    🔑 <strong>Contraseña temporal generada:</strong>
    <code><?php echo htmlspecialchars($_SESSION['flash_reset_pw']); ?></code>
    — comunícala al usuario; deberá cambiarla al entrar.
  </div>
  <?php unset($_SESSION['flash_reset_pw']); endif; ?>
  <div class="card">
  ...
  <div id="resetModal" class="modal-dialog" role="dialog" aria-modal="true">
    ...
    <div class="modal-body">
      ...
      <form id="reset-user-form" method="post" class="row-form">
        ...
        <div style="margin-bottom: 1rem;">
          <label class="form-label">Nueva contraseña (dejar vacío → se genera una temporal aleatoria)</label>
          <input class="form-control" type="password" name="new_password" placeholder="Dejar vacío para contraseña temporal aleatoria">
        </div>
        <div class="form-actions modal-actions mt-2">
