<?php
// db.php incluye config.php y registra el autoloader PSR-4 de App\ (necesario para App\Time, etc.)
...
// Funciones de tiempo unificadas en App\Time (se conservan los nombres por compatibilidad).
function time_to_minutes(?string $time): ?int {
    return \App\Time::toMinutes($time);
}

function minutes_to_hours_formatted(?int $min): string {
    return \App\Time::format($min);
}
